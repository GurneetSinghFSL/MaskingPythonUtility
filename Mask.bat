...
if exist "%BUNDLED_EXE%" (
  "%BUNDLED_EXE%" mask --config "%CONFIG_FILE%"
...
if not defined PY_CMD (
  powershell -NoProfile -Command "Add-Type -AssemblyName PresentationFramework; [System.Windows.MessageBox]::Show('Python runtime not found. Install Python 3.10+ and ensure py or python command is available.', 'Mask Utility - Runtime Error', 'OK', 'Error') | Out-Null"
  set "EXIT_CODE=9009"
  goto :finish
)

call !PY_CMD! -m masking_utility.cli mask --config "%CONFIG_FILE%"
set "EXIT_CODE=!ERRORLEVEL!"
...
if !EXIT_CODE! NEQ 0 (
  powershell -NoProfile -Command "Add-Type -AssemblyName PresentationFramework; [System.Windows.MessageBox]::Show('Mask operation failed with error code !EXIT_CODE!. Check Logs\\masking_utility.log if available.', 'Mask Utility - Error', 'OK', 'Error') | Out-Null"
)
